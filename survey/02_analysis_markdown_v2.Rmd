...
conjoint_data <- readRDS(here::here("survey", "data", "conjoint_data.rds"))
...
```{r main model anger, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
### General model no interactions
f.anger <- chosen ~ Education + Experience + Partisan + Race + Gender + Anger

marginalmeans.anger <- cj(conjoint_data, f.anger, id = ~ResponseId, estimate = "mm")# for the left-right bins

plot(marginalmeans.anger) + 
  labs(x="Marginal Means for the probability to choose a profile")  + 
  geom_point(aes(y=level, x=estimate),size = 2.2) + geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
...
marginalmeans <- cj(conjoint_data_sub, f, id = ~ResponseId, estimate = "amce", by = ~Partisan)# for the left-right bins
marginalmeans$group <- marginalmeans$BY #add a grouping variable for the plot function

plot(marginalmeans) + 
  labs(x="The AMCE to choose a profile, for female respondents only: Immigration arm")  + ggplot2::facet_wrap(~BY, ncol = 4L) +
  geom_point(aes(y=level, x=estimate),size = 2.2) + geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
  scale_color_grey() +
...
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
# The model
f <- chosen ~ Education + Experience + Race + Partisan +  Trust

marginalmeans <- cj(conjoint_data , f, id = ~ResponseId, estimate = "mm", by=~Gender)

plot(marginalmeans) + ggplot2::facet_wrap(~BY, ncol = 4L) +
  labs(x="The MM to choose a profile: for trust")  + 
  geom_point(aes(y=level, x=estimate),size = 2.2) + geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
  scale_color_grey() +
...
plot(marginalmeans) + ggplot2::facet_wrap(~BY, ncol = 4L) +
  labs(x="The MM to choose a profile: for anger")  + 
  geom_point(aes(y=level, x=estimate),size = 2.2) + geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
  scale_color_grey() +
...
marginalmeans <- cj(conjoint_data_sub, f, id = ~ResponseId, estimate = "amce", by=~Party)# for the left-right bins


plot(marginalmeans) + ggplot2::facet_wrap(~BY, ncol = 4L) +
  labs(x="The AMCE to choose a profile: the effect of anger for female candidates only")  + 
...
marginalmeans <- cj(conjoint_data_sub, f, id = ~ResponseId, 
                    estimate = "amce", by=~Party)# for the left-right bins

plot(marginalmeans) + ggplot2::facet_wrap(~BY, ncol = 4L) +
  labs(x="The AMCE to choose a profile: the effect of trust for female candidates only")  + 
  geom_point(aes(y=level, x=estimate),size = 2.2) + 
  geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
...
marginalmeans <- cj(conjoint_data, f, id = ~ResponseId, estimate = "mm", by=~Gender)# for the left-right bins
...
marginalmeans_cov <- cj(conjoint_data, f, id = ~ResponseId, estimate = "mm", by=~Gender)# for the left-right bins
...
plot(marginalmeans) + ggplot2::facet_wrap(~BY, ncol = 4L) +
  labs(x="The mm to choose a profile: the effect of anger for female candidates only")  + 
  geom_point(aes(y=level, x=estimate),size = 2.2) + 
  geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
...
marginalmeans <- cj(conjoint_data_sub, f, id = ~ResponseId, 
                    estimate = "mm", by=~Party)# for the left-right bins

plot(marginalmeans) + ggplot2::facet_wrap(~BY, ncol = 4L) +
  labs(x="The mm to choose a profile: the effect of trust for female candidates only")  + 
  geom_point(aes(y=level, x=estimate),size = 2.2) + 
  geom_linerange(aes(xmin=lower, xmax=upper), size=1.2) +
  geom_vline(xintercept = 0.5, linetype="dashed") +
  theme_set(theme_light(base_family = "Georgia")) +
