...
The first model reported below in the forest plot focuses on the different conjoint attributes without looking at any interactions. All the plots below are very ugly, they're just there to present the results so that we can discuss them. The baselines are no education, caucasian, low for the emotions variables, Democrats, and female. 
...
We thus find that the emotionality of a Tweet has the strongest influence on whether people like a candidate. In addition, respondents seem to slightly prefer female candidates and Democratic candidates. More education and experience in office both have a positive impact on candidate choice. Race is not significant.
...
f <- chosen ~ edu + exp + party + race + trust_cut + gender + arm
x <- cj(conjoint_data, f, id = ~ResponseId, estimate = "mm", by = ~arm)# for the left-right bins
...
We can also look at the variables that we added for the mechanisms. Perhaps respondent's do not like a candidate less but evaluate her as less competent. For instance, let's look at whether respondents think a candidate can get things done in office. 

```{r echo=TRUE}
x <- lm_robust(thingsdone_chosen ~ edu + exp + party + #we thus use a different outcome
                    race + trust_cut + gender * arm, 
                  data=conjoint_data, cluster=ResponseId) 
...
plot_model(x, SE=T, show.values = TRUE,
           ...
           digits = 3) +
   ylim(-.15, .15) + 
  theme_minimal() + ggtitle("Policy Domain and Gender: getting things done")



```
We find that people generally think that women get things done more than men, but this effect is somewhat smaller on immigration. Although the interaction term is not significant. These results are the same for the other outcomes (how committed you are and if you perform under pressure). 
...
* Or we're just simply wrong and policy domain does not influence whether people prefer men or women. I think this is less likely, however, as the signs are in the expected direction. 
